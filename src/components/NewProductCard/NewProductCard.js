...
const NewProductCard = ({ alt, src, tagline, scrollToTop, className }) => {
  ...
  return (
    <div className="new-product-card" onClick={scrollToTop}>
      <Link
        ...
        onClick={() => scrollToTop}
      >
        <RenderSmoothImage src={src} className={className ? `new-product-img ${className}` : "new-product-img"} alt={alt} />
      </Link>
    </div>
  );
};
